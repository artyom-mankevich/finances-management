...
class EthKeysViewSet(
    ...
    def transfer(self, request, *args, **kwargs):
        ...
        eth_keys = raw_get_object_or_404(EthKeys, field='id', value=eth_keys_id)
    ...
    def transactions(self, request, *args, **kwargs):
        addresses = [
            wallet.address for wallet in EthKeys.objects.raw(
                f"SELECT id, address FROM {EthKeys._meta.db_table}"
                f" WHERE user_id=%s",
                [str(self.request.user)]
            )
        ]
    ...
    def get_object(self):
        try:
            ...
            if not id:
                raise IndexError
            return EthKeys.objects.raw(
                f'SELECT * FROM {EthKeys._meta.db_table} WHERE id=%s', [id]
            )[0]
        except IndexError:
